...
function getTaxDataFromInputs(fedTaxableIncome, fedTaxableIncomeOnly, isMarried) {
  const _taxData = isMarried ? taxData.filter(t => t.marriageStatus === "married") : taxData.filter(t => t.marriageStatus === "single")
  const taxDataSorted = _taxData.sort((a, b) => b.fedLevel - a.fedLevel)
  let fedTax = 0
