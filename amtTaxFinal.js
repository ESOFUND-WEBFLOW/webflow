...
function fetchData(url, isState) {
  ...
  xhr.onload = function() {
    if (xhr.status === 200) {
      ...
      try {
        if (isState) {
          stateTaxData = JSON.parse(responseData);
        } else {
          taxData = JSON.parse(responseData);
        }
      } catch (error) {
...
fetchData('https://api.esofund.io/taxes/2022', false)
...
function drawChart() {
  ...
  const stateTaxData = getStateTaxDataFromInputs(state, income, isMarried)
  const stateAmtRate = stateTaxData.stateAmtRate
